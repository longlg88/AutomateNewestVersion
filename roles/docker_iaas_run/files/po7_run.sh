...
image_id=`docker ps | awk '/po7/{print $2}'`

if [ "$image_id" = "$image_name":"$image_tag" ]; then
   echo "same docker container runs here"
else
	docker run -d --name=iaas -p $PO_PORTAL_PORT_1:6776 -p $PO_PORTAL_PORT_2:9008 -p $PO_WEBADMIN_PORT:9736 -p $PO_WEB_PORT:8080 -v /root/automation/po7/proobject7:/proobject7 -e DOMAINNAME=$DOMAINNAME -e TIBERO_IP=$TIBERO_IP -e TIBERO_USER=$TIBERO_USER -e TIBERO_PW=$TIBERO_PW -e TIBERO_PORT=$TIBERO_PORT -it $image_name:$image_tag >> /root/automation/log/progress
fi
